...
const NewsDetailPage: React.FC = () => {
  ...
  const fetchCurrentNews = async () => {
    try {
      setLoading(true);
      const response = await axios.get<News>(`${process.env.REACT_APP_API_URL}/api/news/${newsId}`);
  ...
  try {
    const response = await axios.get<News[]>(`${process.env.REACT_APP_API_URL}/api/news`, {
              ...
              <div className="px-6 pt-6">
                {currentNews.imageUrl ? (
                  <div className="rounded-lg aspect-[4/5] overflow-hidden shadow-md">
                    <img
                      src={`${process.env.REACT_APP_API_URL}${currentNews.imageUrl}`}
