...
const DepartmentManager: React.FC<DepartmentManagerProps> = ({ data, loading, fetchData }) => {
  ...
  const handleDepartmentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingDepartment) {
        await axios.put(`${process.env.REACT_APP_API_URL}/api/unit/${editingDepartment.id}`, formData, { headers });
      } else {
        if (departmentFormData.orderInd === 0) {
          const maxOrder = Math.max(...data.map(d => d.orderInd), 0);
          formData.set('OrderInd', (maxOrder + 1).toString());
        }
        await axios.post(`${process.env.REACT_APP_API_URL}/api/unit`, formData, { headers });
      }
      await fetchData();
      handleCloseModal();
    } catch (error) {
  ...
  const handleDeleteDepartment = async (id: number) => {
    if (confirm('Ви впевнені, що хочете видалити цей відділ?')) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_URL}/api/unit/${id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
