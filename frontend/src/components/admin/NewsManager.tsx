...
const NewsManager: React.FC<NewsManagerProps> = ({ data, loading, fetchData }) => {
  ...
  const handleNewsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingNews) {
        await axios.put(`${process.env.REACT_APP_API_URL}/api/news/${editingNews.id}`, formData, { headers });
      } else {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/news`, formData, { headers });
      }
